...
import { FaHeadphonesAlt, FaList, FaPlus, FaThLarge, FaTrashAlt, FaCubes, FaStar } from "react-icons/fa";
...
function SidebarMenuItem({ id, label, isActive, removable, onClick, onRemove }) {
    ...
    function getMenuPath(menuId) {
        ...
        if (menuId === "fiturxyz") return "/fiturxyz";
        return "/";
    }
    ...
    return (
        <li>
            <Link
                id={id === "dashboard" ? "menu-1" : id === "orders" ? "menu-2" : id === "customers" ? "menu-3" : id === "components" ? "menu-4" : id === "fiturxyz" ? "menu-5" : id}
                to={getMenuPath(id)}
